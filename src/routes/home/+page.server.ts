...
export const load: PageServerLoad = async (event) => {
  ...
  const feed = await sql<Rooms[]>`select image, id, name, description, uuid, tags, owner from rooms limit 9`;
...
export const actions: Actions = {
    search: async(event: any) => {
      ...
      const [ feed ] = await sql<Rooms[]>`select image, id, name, description, uuid, tags from rooms where ${query}`;
